class platform_services_dns::server {
  ...
  # fail if interfaces have no ips
  unless (is_ip_address($::platform_services_dns::ipaddress_serv) and is_ip_address($::platform_services_dns::ipaddress_sync) and is_ip_address($::platform_services_dns::ipaddress_stor)) {
    fail("dns server must have all interfaces up and running")
  }
  ...
  if $::platform_services::manage_front_ips {
    platform_services_dns::server::zone{
      "${::mpc_project}.${::mpc_bu}.mpc":
        require => Class['::platform_services::front_ip'],
